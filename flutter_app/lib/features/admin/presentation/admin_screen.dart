...
class _AdminScreenState extends ConsumerState<AdminScreen> with TickerProviderStateMixin {
  ...
  final TextEditingController _broadcastMinBalanceCtrl = TextEditingController();
  final TextEditingController _broadcastLastSeenAfterCtrl = TextEditingController();
          ...
          child: ListView(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Telegram ID (optional)'),
              ),
            ...
            (user) => Card(
              child: ListTile(
                ...
                leading: CircleAvatar(
                  backgroundImage: user.photoUrl.trim().isEmpty ? null : NetworkImage(user.photoUrl),
                  child: user.photoUrl.trim().isEmpty
                      ? Text(user.displayName.trim().isNotEmpty ? user.displayName.trim().substring(0, 1).toUpperCase() : 'U')
                      : null,
                ),
                title: Text(user.displayName),
                subtitle: Text('@${user.username.isEmpty ? 'no_username' : user.username} • TG ${user.telegramId}'),
                trailing: Column(
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text('Create broadcast', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 10),
                DropdownButtonFormField<String>(
                  value: _broadcastAudience,
                  items: const [
                    DropdownMenuItem(value: 'all', child: Text('All active users')),
                    DropdownMenuItem(value: 'selected', child: Text('Selected user IDs')),
                    DropdownMenuItem(value: 'filter', child: Text('Filter-based')),
                  ],
                  onChanged: (value) {
                    if (value == null) return;
                ...
                if (_broadcastAudience == 'filter') ...[
                  TextField(
                    ...
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(labelText: 'Min balance (optional)'),
                  ),
                  const SizedBox(height: 10),
                  TextField(
                ...
                Align(
                  ...
                  child: TextButton.icon(
                    onPressed: () {
                      setState(() => _broadcastButtons.add(_BroadcastButtonDraft(text: '', url: '')));
                    },
                ...
                FilledButton(
                  ...
                  child: Text(_broadcastCreateBusy ? 'Creating…' : 'Create broadcast'),
                ),
                if ((_broadcastsError ?? '').trim().isNotEmpty) ...[
                  const SizedBox(height: 8),
                  Text(_broadcastsError!, style: const TextStyle(color: Colors.red)),
                ],
              ],
            ),
          ),
            ...
            (item) => Card(
              child: ListTile(
                ...
                trailing: item.status == 'pending'
                    ? OutlinedButton(
                        onPressed: _broadcastStartBusyId == item.id ? null : () => _startBroadcast(item.id),
                        child: Text(_broadcastStartBusyId == item.id ? 'Starting…' : 'Start'),
                      )
                    : null,
              ),
            ),
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Parser sources', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 10),
                TextField(
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'Source input (URL or channel)'),
                ),
                const SizedBox(height: 8),
                DropdownButtonFormField<String>(
                ...
                const SizedBox(height: 8),
                FilledButton(
                  onPressed: _parserCreateSourceBusy ? null : _createParserSource,
                  child: Text(_parserCreateSourceBusy ? 'Saving…' : 'Add source'),
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    ...
                    const Spacer(),
                    TextButton.icon(
                      onPressed: _parserSourcesLoading ? null : _loadParserSources,
                      icon: const Icon(Icons.refresh_rounded),
                      label: const Text('Refresh'),
                    ),
                  ],
                ...
                ..._parserSources.map(
                  (source) => Card(
                    child: ListTile(
                      title: Text(source.title.trim().isEmpty ? '#${source.id}' : source.title),
                      subtitle: Text('${source.sourceType} • ${source.input}\nLast parsed: ${formatDateTime(source.lastParsedAt)}'),
                      isThreeLine: true,
                      trailing: Wrap(
                        spacing: 6,
                        children: [
                          OutlinedButton(
                            onPressed: _parserSourceBusyId == source.id ? null : () => _parseSource(source.id),
                            child: Text(_parserSourceBusyId == source.id ? '…' : 'Parse'),
                          ),
                          OutlinedButton(
                            onPressed: _parserSourceBusyId == source.id ? null : () => _toggleParserSource(source),
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Quick parse', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 10),
                TextField(
                  controller: _parserQuickInputCtrl,
                  decoration: const InputDecoration(labelText: 'URL or channel'),
                ),
                const SizedBox(height: 8),
                DropdownButtonFormField<String>(
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('${item.name.isEmpty ? 'Untitled' : item.name} • ${item.status}'),
            const SizedBox(height: 4),
            Text('${item.sourceType} • parsed ${formatDateTime(item.parsedAt)}'),
            if (item.location.trim().isNotEmpty) Text('Location: ${item.location}'),
            if (item.parserError.trim().isNotEmpty)
              Padding(
                padding: const EdgeInsets.only(top: 6),
                child: Text(item.parserError, style: const TextStyle(color: Colors.red)),
              ),
            if (item.status == 'pending') ...[
              ...
              TextField(
                ...
                minLines: 2,
                maxLines: 5,
                decoration: const InputDecoration(labelText: 'Media URLs (one per line)'),
              ),
              const SizedBox(height: 10),
              Wrap(
                spacing: 8,
                ...
                children: [
                  FilledButton.tonal(
                    onPressed: _parserGeocodeBusyId == item.id ? null : () => _geocodeDraft(item),
                    child: Text(_parserGeocodeBusyId == item.id ? 'Geocoding…' : 'Geocode'),
                  ),
                  FilledButton(
                    onPressed: _parserImportBusyId == item.id ? null : () => _importParsed(item),
                    child: Text(_parserImportBusyId == item.id ? 'Importing…' : 'Import to events'),
                  ),
                  OutlinedButton(
                    onPressed: _parserRejectBusyId == item.id ? null : () => _rejectParsed(item.id),
                    child: Text(_parserRejectBusyId == item.id ? 'Rejecting…' : 'Reject'),
                  ),
                  OutlinedButton(
                    onPressed: _parserDeleteBusyId == item.id ? null : () => _deleteParsed(item.id),
                    child: Text(_parserDeleteBusyId == item.id ? 'Deleting…' : 'Delete'),
                  ),
                ],
              ),
            ] else ...[
              ...
              Row(
                children: [
                  if (item.importedEventId != null)
                    FilledButton.tonal(
                      onPressed: () => context.push('/event/${item.importedEventId}'),
                      child: Text('Open event #${item.importedEventId}'),
                    ),
                  const SizedBox(width: 8),
                  OutlinedButton(
                    onPressed: _parserDeleteBusyId == item.id ? null : () => _deleteParsed(item.id),
                    child: Text(_parserDeleteBusyId == item.id ? 'Deleting…' : 'Delete from DB'),
                  ),
                ],
              ),
            ],
  ...
  Future<void> _openUserDetail(int userId) async {
    ...
    try {
      final detail = await ref.read(adminRepositoryProvider).getUser(token: token, id: userId);
      setState(() {
        _userDetail = detail;
      });
    } catch (error) {
          ...
          child: SingleChildScrollView(
            child: _userDetailLoading
                ...
                : (_userDetailError ?? '').isNotEmpty
                    ? Padding(
                        padding: const EdgeInsets.all(12),
                        child: Text(_userDetailError!, style: const TextStyle(color: Colors.red)),
                      )
                    : detail == null
                        ? const Padding(
                        ...
                        : Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(detail.user.displayName, style: Theme.of(context).textTheme.titleLarge),
                              const SizedBox(height: 6),
                              Text('TG: ${detail.user.telegramId} • @${detail.user.username}'),
                              const SizedBox(height: 6),
                              Text('Balance: ${detail.user.balanceTokens} GT'),
                              Text('Last seen: ${formatDateTime(detail.user.lastSeenAt)}'),
                              const SizedBox(height: 10),
                              if (!detail.user.isBlocked)
                                TextField(
                                  controller: _blockReasonCtrl,
                                  decoration: const InputDecoration(labelText: 'Block reason (optional)'),
                                ),
                              const SizedBox(height: 8),
                              FilledButton(
                              ...
                              const SizedBox(height: 12),
                              Text('Created events (${detail.createdEvents.length})',
  ...
  Future<void> _toggleBlockUser(AdminUser user) async {
    ...
    try {
      if (user.isBlocked) {
        await ref.read(adminRepositoryProvider).unblockUser(token: token, id: user.id);
      } else {
      ...
      await _loadUsers();
      final detail = await ref.read(adminRepositoryProvider).getUser(token: token, id: user.id);
      setState(() {
        _userDetail = detail;
      });
    } catch (error) {
  ...
  Future<void> _loadBroadcasts() async {
    ...
    try {
      final response = await ref.read(adminRepositoryProvider).listBroadcasts(token: token);
      setState(() {
  ...
  Future<void> _startBroadcast(int id) async {
    ...
    try {
      await ref.read(adminRepositoryProvider).startBroadcast(token: token, id: id);
      await _loadBroadcasts();
    } catch (error) {
      _handleAdminError(error, setter: (value) => _broadcastsError = value);
    } finally {
  ...
  Future<void> _loadParserSources() async {
    ...
    try {
      final response = await ref.read(adminRepositoryProvider).listParserSources(token: token);
      setState(() {
  ...
  Future<void> _parseSource(int sourceId) async {
    ...
    try {
      final result = await ref.read(adminRepositoryProvider).parseSource(token: token, id: sourceId);
    ...
    try {
      ...
      setState(() {
        ...
        for (final item in response.items) {
          _parserDrafts.putIfAbsent(item.id, () => _ParserDraft.fromParsed(item));
        }
      });
    } catch (error) {
      _handleAdminError(error, setter: (value) => _parserError = value);
  ...
  Future<void> _geocodeDraft(AdminParsedEvent item) async {
    ...
    final query = draft.addressCtrl.text.trim().isNotEmpty
        ...
        : (draft.titleCtrl.text.trim().isNotEmpty ? draft.titleCtrl.text.trim() : item.location.trim());
    if (query.isEmpty) {
    ...
    try {
      final response = await ref.read(adminRepositoryProvider).geocode(token: token, query: query, limit: 1);
  ...
  Future<void> _rejectParsed(int id) async {
    ...
    try {
      await ref.read(adminRepositoryProvider).rejectParsedEvent(token: token, id: id);
      await _loadParsedEvents();
    } catch (error) {
      _handleAdminError(error, setter: (value) => _parserError = value);
    } finally {
  ...
  Future<void> _deleteParsed(int id) async {
    ...
    try {
      await ref.read(adminRepositoryProvider).deleteParsedEvent(token: token, id: id);
      await _loadParsedEvents();
    } catch (error) {
      _handleAdminError(error, setter: (value) => _parserError = value);
    } finally {
...
class _ParserDraft {
  ...
  factory _ParserDraft.fromParsed(AdminParsedEvent item) {
    ...
    final eventLinks = _dedupeTrimmed(item.links.where((link) => !_isImageLink(link)).toList());

    return _ParserDraft(
      titleCtrl: TextEditingController(text: item.name),
      descriptionCtrl: TextEditingController(text: item.description),
      startsAtCtrl: TextEditingController(text: item.dateTime?.toUtc().toIso8601String() ?? ''),
...
bool _isImageLink(String value) {
  ...
  if (RegExp(r'\.(jpg|jpeg|png|webp|gif|bmp|svg)(\?|$)').hasMatch(trimmed)) return true;
  if (trimmed.contains('/photo') || trimmed.contains('/image') || trimmed.contains('/img')) return true;
  return false;
}
